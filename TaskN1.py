...
if output == 'y':
    for i in range(len(sid)):
        print(f'{sid[i]}\n') #Here you can change the output method and output, for example, like this: ( print(f'{ip} - sid={sid[i]}\n') )
    input('Press ENTER to exit')
else:
    with open('log-out.txt', 'w') as ouf:
        for i in range(len(sid)):
            ouf.write(f'{sid[i]}\n') #Here you can change the output method and output, for example, like this: ( ouf.write(f'{ip} - sid={sid[i]}\n') )
